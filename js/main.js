...
jQuery(document).ready(function($) {
	...
	if (jQuery(".demo_store").length >= 1) {
		...
		if(jQuery("#slidingbar-area").outerHeight()  > 0) {
			...
			jQuery("#wrapper").css('margin-top', jQuery(".demo_store").outerHeight() - 3);
	...
	if(jQuery('.sticky-header').length >= 1) {
		...
		jQuery('#sliders-container').waypoint(function(direction) {
			...
			if(direction === "down") {
				...
				logo_max_width = jQuery('.sticky-header .logo img.normal_logo').data("max-width");											
	...
	jQuery(window).on('resize', function() {
		...
		if (jQuery(".demo_store").length >= 1) {
			...
			jQuery("#wrapper").css('margin-top', jQuery(".demo_store").height()+parseInt(jQuery(".demo_store").css('padding-top').replace('px',''))+parseInt(jQuery(".demo_store").css('padding-bottom').replace('px','')));

			if(jQuery('.sticky-header').length >= 1) {

				jQuery(".sticky-header").css('margin-top', jQuery(".demo_store").height()+parseInt(jQuery(".demo_store").css('padding-top').replace('px',''))+parseInt(jQuery(".demo_store").css('padding-bottom').replace('px','')));
	...
	calcTabsLayout('.project-content .tabs-horizontal .tabset,.post-content .tabs-horizontal .tabset');
	...
	jQuery('.woocommerce #calc_shipping_country, .woocommerce .country_select, #bbp_stick_topic_select, #bbp_topic_status_select, #bbp_forum_id, #bbp_destination_topic,.woocommerce select#calc_shipping_state, .woocommerce select.state_select').wrap('<div class="avada-select-parent"></div>').after('<div class="select-arrow">&#xf107;</div>');
...
//Show Website Password Pin
	jQuery("#showpassword").click(function(){
		jQuery('#password_pin_validation').css('display','block');
		jQuery.post( "../wp-content/themes/Avada/custom_pin_password_checker.php", function(obj){
			console.log(obj);			
		});
		return false;
	});

	jQuery("#password_pin").submit(function(){
		
	    jQuery.post( "../wp-content/themes/Avada/custom_pin_password_checker.php", jQuery( this ).serialize(),
	    function(obj){
			if(obj==1){
				jQuery('#password_pin_validation').css('display','none');
				jQuery("#showpassword").hide();
				jQuery(".site_password").show();
			}
			if(obj==0){
...
//Get Website Wordpress Version 
	jQuery("#fetch_wp_version").click(function(){
		site_url = jQuery( this ).attr("data-id");
		jQuery(".wp_version_loader").css( "display", "inline-block" );
		jQuery.post( "../wp-content/themes/Avada/custom_wordpress_version_checker.php", { siteurl: site_url },
			function(obj){
				if(obj=='empty'){
					jQuery("#wp_version_note").show();
				}else{
					jQuery("#the_wp_version").val( obj );
				}				
				console.log(obj);
				jQuery(".wp_version_loader").hide();		
			});
		return false;
	});