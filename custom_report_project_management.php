...
$projects = $wpdb->get_results("SELECT p.* FROM wp_custom_project as p LEFT JOIN wp_custom_timesheet as t ON p.project_client = t.task_label WHERE p.project_status = 'unarchived'AND STR_TO_DATE(date_now, '%d/%m/%Y') BETWEEN STR_TO_DATE('01/$month/$year', '%d/%m/%Y') AND STR_TO_DATE('31/$month/$year', '%d/%m/%Y') ORDER BY project_client ASC"); $project_names = $wpdb->get_results("SELECT DISTINCT project_name FROM {$table_name_project}");

$date_filter = "STR_TO_DATE(date_now, '%d/%m/%Y') BETWEEN STR_TO_DATE('01/$month/$year', '%d/%m/%Y') AND STR_TO_DATE('31/$month/$year', '%d/%m/%Y')";
		...
		<div class="tab-hold tabs-wrapper">
			<div class="full_width">
				<ul id="tabs" class="tabset tabs">
					...
					<li class="tabs_li"><a href="#seo">Monthly Ongoing</a></li>
					...
					<div class="display_main">
						<div class="section current">
							...
							<?php 
								...
								$seo_current_projects = ProjectManagenentQueries($date_filter);
								...
								<div id="display_note_<?php echo $project->ID; ?>" class="display_note display_list hide_list_<?php echo $project->ID; ?>">
									<div class="info_div">										
										<p id="first_column_<?php echo $project->ID; ?>" class="client_info first_column column"><?php echo $project->client_name; ?></p>								
										<p id="second_column_<?php echo $project->ID; ?>" class="second_column column"><?php echo round_quarter($project->seo_plan); ?></p>																								
										<p id="third_column_<?php echo $project->ID; ?>" class="third_column column"><?php echo round_quarter($project->webdev_plan); ?></p>																								
										<p id="fourth_column_<?php echo $project->ID; ?>" class="fourth_column column"><?php echo round_quarter($project->seo_hours); ?></p>
										<p id="fifth_column_<?php echo $project->ID; ?>" class="fifth_column column"><?php echo round_quarter($project->dev_hours); ?></p>													
										<p id="sixth_column_<?php echo $project->ID; ?>" class="sixth_column column"><?php echo round_quarter($project->con_hours); ?></p>
										<p id="seventh_column_<?php echo $project->ID; ?>" class="seventh_column column"><?php echo round_quarter($project->total_hours); ?></p>
										<p id="eighth_column_<?php echo $project->ID; ?>" class="eighth_column column"><?php echo  $project_expense;  ?></p>
										<p id="ninth_column_<?php echo $project->ID; ?>" class="ninth_column column <?php echo $revenue_class; ?>"><?php echo round($project_revenue); ?></p>
										<p id="tenth_column_<?php echo $project->ID; ?>" class="tenth_column column"><?php echo $project->budget; ?></p>
									</div>
									<div style="display:none" id="project_notes_<?php  echo $project->ID; ?>" class="project_notes">
										<p style="float:left"><strong><?php echo $project->client_name .":&nbsp";?></strong></p>
										<p style="float:left"><?php echo '<strong>SEO: </strong>' . $project->project_description . '<br/>' . '<strong>DEV: </strong>' . $prject_description_dev; ?></p>
									</div>
								</div>
							<?php } ?>
							</div>
