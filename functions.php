...
function pm_current_seo_filter($filter_details){
	...
	// 	}
	$seo_current_projects = ProjectManagenentQueries($seo_filter_query);

	foreach($seo_current_projects as $project){

		$project_expense = $project->persons_rate + $project->extra_expenses;
		$project_revenue = ($filter_month == 'null')? ($project->budget * 12) - $project_expense : $project->budget - $project_expense;
		$budget = ($filter_month == 'null')? ($project->budget * 12) : $project->budget;
		// $project_revenue = $project->budget - $project_expense;		
		...
		$filter_current_seo_array['current_seo_filter_details'][] = $project->ID ."_". $project->client_name ."_". round_quarter($project->seo_plan) ."_". round_quarter($project->webdev_plan) ."_". round_quarter($project->seo_hours) ."_". round_quarter($project->dev_hours) ."_". round_quarter($project->con_hours) ."_". round_quarter($project->total_hours) ."_". $project_expense ."_". round($project_revenue) ."_". $revenue_class ."_". $project->project_description ."_". $budget;	
...
function CheckedApprovalByPerson($data){
	...
	if($approved_by_person_status == 1){
		...
		if($invoice_info->admin_approval == 1){
			SendInvoicePDFtoAdmin($invoice_id);
		}
...
function CheckedApprovalByAdmin($data){
	...
	if($approved_by_admin_status == 1){
		...
		if($invoice_info->person_approval == 1){
			SendInvoicePDFtoAdmin($invoice_id);
		}
	}else{
...
function SendInvoicePDFtoAdmin($invoice_id){
	require_once('../../../download-invoice-pdf/fpdf.php' );
	global $wpdb;
    ...
    $fullname = strtolower($person_info->person_first_name) . "-" . strtolower($person_info->person_last_name);
    ...
    $filename = 'auto-invoice-'.$fullname.'-'.$invoice_info->date.'.pdf';
    $file_path = '../../../download-invoice-pdf/auto-invoices-files/'.$filename;
	...
	$body = "
	<h1>Hello ".$admin_info->person_fullname.",</h1>
	<p>".$person_name."'s Invoice for the ". date("F", mktime(0, 0, 0, $dates[0], 10))." ". $dates[1] ."</p>";

	$to = $admin_info->person_email;
	$subject = "".$person_name."'s Invoice for ".date("F", mktime(0, 0, 0, $dates[0], 10))." ". $dates[1] .". ";

	$attachments = array( $file_path );

	$headers = array('Content-Type: text/html; charset=UTF-8','From: '.$person_name.'\'s Splan Auto Invoice <info@seowebsolutions.se'); 
...
function ProjectManagenentQueries($date_filter){
	global $wpdb;
	return $wpdb->get_results("SELECT 
client.ID as ID,
client.client_name,
mp.monthly_seo_hours as seo_plan,
mp.monthly_webdev_hours as webdev_plan,
revenue_table.seo_hours as seo_hours,
revenue_table.dev_hours as dev_hours,
revenue_table.con_hours as con_hours,
revenue_table.total_hours as total_hours,
revenue_table.persons_rate as persons_rate,
revenue_table.extra_expenses as extra_expenses,
revenue_table.budget as budget
FROM wp_custom_client as client
LEFT JOIN wp_custom_monthly_plan as mp
ON mp.monthly_name = client.client_monthly_plan
RIGHT JOIN (SELECT
c.ID as client_id, 
t.task_label,
-- p.project_name,
p.project_description as project_description,
ROUND(SUM(if(t.task_project_name = 'Monthly Ongoing SEO' AND t.task_person <> 'Quima May Renegado' AND ts.task_billable = 1, time_to_sec(t.task_hour) / (60 * 60), 0)), 2) as seo_hours,
ROUND(SUM(if(t.task_project_name = 'Monthly Ongoing Dev', time_to_sec(t.task_hour) / (60 * 60), 0)), 2) as dev_hours,
ROUND(SUM(if(t.task_project_name = 'Monthly Ongoing SEO' AND t.task_person = 'Quima May Renegado' AND ts.task_billable = 0, time_to_sec(t.task_hour) / (60 * 60), 0)), 2) as con_hours,
ROUND(SUM(if((t.task_project_name = 'Monthly Ongoing SEO' OR t.task_project_name = 'Monthly Ongoing Dev'), time_to_sec(t.task_hour) / (60 * 60), 0)), 2) as total_hours,
ROUND(SUM(if(t.task_project_name = 'Monthly Ongoing SEO' OR t.task_project_name = 'Monthly Ongoing Dev', time_to_sec(t.task_hour) / (60 * 60) * user.person_hourly_rate, 0)), 2) as persons_rate,
(p.project_default_expenses + p.project_extra_expenses + mp.monthly_seo_extra_expense + mp.monthly_dev_extra_expense) as extra_expenses,
mp.monthly_budget as budget
FROM wp_custom_timesheet as t
LEFT JOIN wp_custom_task as ts
ON ts.task_name = t.task_name
RIGHT JOIN wp_custom_project as p
ON p.project_client = t.task_label AND t.task_project_name = p.project_name
LEFT JOIN wp_custom_client as c
ON t.task_label = c.client_name
LEFT JOIN wp_custom_monthly_plan as mp
ON c.client_monthly_plan = mp.monthly_name
LEFT JOIN wp_custom_person as user
ON t.task_person = user.person_fullname
WHERE ".$date_filter."
AND t.task_label <> 'SEOWeb Solutions'
AND p.project_status = 'unarchived'
AND p.project_invoice_method = 1
GROUP BY t.task_label) as revenue_table
